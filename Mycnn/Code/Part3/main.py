...
def createData(data_dir= 'train',batch_size=32,num_train = 500,num_test = 120):
    # 数据加载
    ...
    train_set = CustomDataset(
        data_dir='train', transform=data_transform, pic_per_class=num_train, needTrain=True,total=total)
    train_loader = DataLoader(train_set, batch_size=batch_size,
                              shuffle=True, num_workers=4)

    test_set = CustomDataset(
        data_dir='train', transform=data_transform, pic_per_class=num_test, needTrain=False,total=total)
...
# for interview
def interview_test_data(data_dir= 'train',batch_size = 32 , num_test = 240):
    data_transform = transforms.Compose([
        transforms.Resize((28, 28)),
        transforms.ToTensor(),
    ])
    test_set = CustomDataset(data_dir=data_dir, transform=data_transform,pic_per_class=num_test,needTrain=False,total=num_test)
...
if __name__ == '__main__':
    ...
    # test_loader = interview_test_data(data_dir='train',batch_size=32,num_test=240)
    # loss_fn = nn.CrossEntropyLoss()
    # validate(model,loss_fn,test_loader,True)
