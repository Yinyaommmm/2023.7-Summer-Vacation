...
def load_data(data_dir, num_train_samples=500, num_test_samples=120):
    ...
    X_train = np.array(X_train).reshape(
        num_train_samples*char_class_num, pic_size**2, 1)
   ...
   # 将图像数据归一化到 [0, 1] 的范围
    ...
    y_train = np.eye(char_class_num)[y_train].reshape(
        num_train_samples*char_class_num, char_class_num, 1)
...
def data_shuffle(trainSet, trainLabelSet):
    assert (len(trainSet) == len(trainLabelSet))
    random_indices = np.random.permutation(len(trainSet))
    trainSet = trainSet[random_indices]
    trainLabelSet = trainLabelSet[random_indices]
...
def save_model(nw: Layer.Network, filename):
